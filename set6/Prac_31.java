...
try (FileReader fr = new FileReader("input.txt");
	...
	FileWriter fw = new FileWriter("output_char.txt")) { int c;
...
try (FileInputStream fis = new FileInputStream("input.txt"); FileOutputStream fos = new FileOutputStream("output_byte.txt")) { byte[] buffer = new byte[1024];
int bytesRead;
...
// Use BufferedReader and BufferedWriter to read from console and write to a file try (BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
BufferedWriter bw = new BufferedWriter(new FileWriter("console_output.txt")))
{
System.out.println("Enter text (type 'exit' to finish):"); String line;
while (!(line = br.readLine()).equalsIgnoreCase("exit")) { bw.write(line);
bw.newLine();
}
